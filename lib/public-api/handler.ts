...
export async function handlePricingRequest(
  ...
  const refundQuota = async () => {
    await db
      ...
      `)
      .catch((err) => console.error(`${TAG} quota refund failed`, err));
  };
  ...
  try {
    ...
    // Identical in both versions, so it is built once.
    const metrics = {
      domain_rating:
      ...
      // "-" is a scraper placeholder for "unknown", stored on 23,263 domains.
      // Returned verbatim it breaks the documented contract: the docs promise
      // a country name or null, and a client rendering country ?? "Unknown"
      // prints a bare dash instead.
    ...
    const body = buildPricingBody(domain, offers, rates, nicheFilter, metrics);

    return finish(jsonOk(body, { ...headers, "X-API-Version": "1" }));
  } catch (err) {
